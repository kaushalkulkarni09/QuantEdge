# ssvi_model.py Volatility Project
...
class HybridVolatilitySurfaceModel:
    ...
    def fetch_option_data(self, ticker_symbol: str, num_expirations: int = 3) -> Optional[pd.DataFrame]:
        """
    ...
    def calibrate_ssvi_model(self, num_generations: int = 200) -> Optional[dict[str, Union[list[float], float]]]:
        """
    ...
    def calibrate_sabr_model(self) -> None:
        ...
        for T_val in unique_TTMs:
            ...
            alpha_init = np.mean(market_vols[np.isclose(strikes, self.S0, atol=0.1*self.S0)]) if len(market_vols) > 0 else 0.2
...
if __name__ == "__main__":
    ...
    market_option_data = hybrid_model.fetch_option_data(ticker_symbol, num_expirations=3)
